...
PostPost provides analytics for LinkedIn posts, including:
...
When you create a post via PostPost, the response includes the platform-specific post ID. For LinkedIn, this is the URN.
...
const postDetails = await fetch(`https://api.postpost.dev/api/v1/get-post/${postGroupId}`, {
